...
Mat Screenshot::getScreenshot()
{
    ...
    // 创建图像
    Mat screenshot = Mat(m_height, m_width, CV_8UC4, m_screenshotData);
...
/* 获取屏幕缩放值 */
double Screenshot::getZoom(HWND hWnd)
{
    HMONITOR hMonitor = MonitorFromWindow(hWnd, MONITOR_DEFAULTTONEAREST);
    // 获取监视器逻辑宽度
    MONITORINFOEX monitorInfo;
...
LPVOID Screenshot::getCurrentSCData() 
{
    return m_screenshotData;
}

HDC Screenshot::getCurrentScreenDCData()
{
    return m_screenDC;
}
...
{
    DeleteObject(m_screenDC);
    DeleteObject(m_compatibleDC);
